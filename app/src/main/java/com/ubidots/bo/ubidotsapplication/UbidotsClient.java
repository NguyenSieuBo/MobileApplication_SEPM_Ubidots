...
public class UbidotsClient {
    ...
    public void handleUbidots(String varId, String apiKey, long date, final UbiListener listener) {

        final List<Value> results = new ArrayList<>();

        OkHttpClient client = new OkHttpClient();
        Request req = new Request.Builder().addHeader("X-Auth-Token", apiKey)
                .url("http://things.ubidots.com/api/v1.6/variables/" + varId + "/values" + "?start=" + date)
                .build();
